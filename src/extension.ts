...
const searchIncludedFiles = (document: vscode.TextDocument, varName: string, visited: Set<string> = new Set()): string => {
            ...
            for (const callMatch of funcMatches) {
                ...
                for (const ch of argsPart) {
                    if (ch === '(') argDepth++;
                        ...
                        if (currentArg.trim()) actualArgs.push(currentArg.trim());
                        currentArg = '';
                        continue;
                    }
                    currentArg += ch;
                ...
                let funcData: string | null = null;
                let bestMatch: {key: string, data: string, matchScore: number} | null = null;
                ...
                for (const k of funcKeys) {
                    ...
                    if (params.length === 0 && actualCount > 0) continue;
                    if (params.length > actualCount && !params.some(p => p.includes('...'))) continue;
                ...
                if (bestMatch) {
                    ...
                    let fallbackMatch: {key: string, data: string, minDiff: number} | null = null;
                    for (const k of funcKeys) {
                        const data = CAPL_FUNCTIONS[k];
                        const syntaxLine = data.split('\n').find(l => l.includes('Syntax:'));
                        if (!syntaxLine) continue;
...
const fullExp = params[i].trim();
                        ...
                        if (arg.toLowerCase() === 'this') continue;
